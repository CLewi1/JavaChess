package com.clewi.javachess.pieces;

import com.clewi.javachess.model.Board;

public class Bishop extends Piece {

    public Bishop(int x, int y, boolean is_white, String file_path, Board board, boolean is_captured) {
        super(x, y, is_white, file_path, board, is_captured, "Bishop", false);
    }
    
    @Override
    public boolean canMove(int destination_x, int destination_y) {
        // Check if board reference exists
        if (board == null) {
            System.out.println("Board is null in Bishop.canMove");
            return false;
        }
        
        int x = this.getX();
        int y = this.getY();
        
        // Check if the destination is within bounds
        if (!isWithinBounds(destination_x, destination_y)) {
            return false;
        }

        // Check if the bishop is moving diagonally
        if (Math.abs(destination_x - x) != Math.abs(destination_y - y)) {
            return false;
        }
        
        // Check if destination has a piece of the same color
        if (isOccupiedBySameColor(destination_x, destination_y)) {
            return false;
        }
        
        // Check if path is clear
        if (!isPathClear(x, y, destination_x, destination_y)) {
            return false;
        }

        return true;
    }
}
